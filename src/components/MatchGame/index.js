...
class MatchGame extends Component {
  ...
  startGame = () => {
    this.intervalId = setInterval(this.onTimeCount, 1000)
  }

  onChangeTab = uniqueTabId => {
    this.setState({activeTabId: uniqueTabId})
  }

  onClickEachItem = eachItemId => {
    const {questionImg} = this.state
    const {id} = questionImg
    if (eachItemId === id) {
      const index = Math.ceil(Math.random() * imagesList.length)
      this.setState(prevState => ({
        score: prevState.score + 1,
        questionImg: imagesList[index],
      }))
    } else {
  ...
  renderOnGameProgress = () => {
    const {activeTabId, questionImg} = this.state
    ...
    return (
      <div className="bg-container">
        <img src={questionImg.imageUrl} alt="match" className="math-image" />
        ...
        <ul className="thumbnails-container">
          {filteredImgList.map(eachItem => (
            <EachItem
              key={eachItem.id}
              itemDetails={eachItem}
              onClickEachItem={this.onClickEachItem}
            />
          ))}
        </ul>
      </div>
  ...
  renderGameViews = () => {
    ...
    return <div className="app-container">{this.renderOnGameProgress()}</div>
  }

  render() {
    const {score, timeInSec, isGameInProgress} = this.state
    const timeObj = {presentScore: score, remainingTime: timeInSec}
    console.log(isGameInProgress)

    return (
      <div className="app-container">
        <NavBar navBarDetails={timeObj} />
        {isGameInProgress ? this.renderGameViews() : this.renderScoreCardView()}
      </div>
    )
  }
}
